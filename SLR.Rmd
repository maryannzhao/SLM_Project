...
movie.pred <- augment(movies, newdata=mill, type.predict = "response")
...
#mill2 <- data.frame(budget = c(1000000))
#predict.lm(movies, newdata = mill2, interval = "confidence", level = .95)
...
ggplot(movie_pred, aes(x =log.bud2.budget., y =log.bud2.popularity.)) + geom_point() +
stat_smooth(method = "lm", se = FALSE) +
geom_ribbon(aes(ymin = lower_PI, ymax = upper_PI), alpha = .2) +
geom_ribbon(data = movie.pred, aes(ymin = lower_CI, ymax = upper_CI), alpha = .2, fill = "red")
