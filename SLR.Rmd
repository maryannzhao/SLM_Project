...
ggplot(movie_WH, aes(x = bud.log, y =pop.log)) + geom_point() +
  stat_smooth(method = "lm", se = FALSE) +
  geom_ribbon(aes(ymin = lower_PI, ymax = upper_PI), alpha = .2) +
  geom_ribbon(data = movie_WH, aes(ymin = lower_CI, ymax = upper_CI), alpha = .2, fill = "red") +
  xlab("log(budget)") + ylab("log(popularity)")

movie_sch <- broom::augment(movies) %>%
mutate(.se.pred = sqrt(movie_sig^2 + .se.fit^2)) %>%
mutate(lower_PI = .fitted - crit_Sch*.se.pred,
upper_PI = .fitted + crit_Sch *.se.pred,
lower_CI = .fitted - crit_Sch * .se.fit,
upper_CI = .fitted + crit_Sch * .se.fit)

movie_sch %>% head()

ggplot(movie_sch, aes(x = bud.log, y =pop.log)) + geom_point() +
  stat_smooth(method = "lm", se = FALSE) +
  geom_ribbon(aes(ymin = lower_PI, ymax = upper_PI), alpha = .2) +
  geom_ribbon(data = movie_sch, aes(ymin = lower_CI, ymax = upper_CI), alpha = .2, fill = "red") +
  xlab("log(budget)") + ylab("log(popularity)")
```
