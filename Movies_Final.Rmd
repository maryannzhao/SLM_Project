...
ggplot(data = test, aes(x=pop.log, y=ridge.pred)) + 
  ...
  scale_colour_manual(name  ="Model Type", values=c("yellow", "orange", "green"),
                          labels=c("Ridge Regression", "Lasso", "MLR"))
```
Figure 1. Comparison of MLR, Ridge Regression and Lasso models.  Ridge Regression (yellow) and Lasso (orange) display a comparable comparable relationship between observed popularirty and predicted popularity where both predict similar values as the observed. MLR (green) on the otherhand appears to underpredict the popularity 

From the plots, it is clear that using Ridge Regression or Lasso on our data effectively reduced the variation. The data points are transformed from being cloud-like to a more linear spread. Interestingly, the models produced by Ridge Regression and Lasso are quite similar. We notice that Lasso and Ridge Regression have both decreased the coefficients of language, Action, Adventure and Drama, relative to the previous Multiple Linear Regression model, indicating  that these variables may be less important for our predictions.
